...
const createCollege = async function (req, res) {
    try {
      ...
      if (!name || !isValid.sortName(name))
       ...
       if(checkName) return res.status(400).send({status:false,message:"name has already ragister"})
      ...
      // resistor college
      let register = await collegeModel.create(data)
      const output={}
       output.name=register.name
       output.fullName=register.fullName
       output.logoLink=register.logoLink
       output.isDeleted=register.isDeleted
...
const getCollege=async function(req,res){
  ...
  if(!data || !collegeName) return res.status(400).send({status:false,message:"coEnter the llege name in collegeName"})
